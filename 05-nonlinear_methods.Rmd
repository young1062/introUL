...
## Kernels and the Kernel Trick {#sec-kernel-trick}
...
Let $\vec{w}_1,\dots,\vec{w}_k\in\mathbb{R}^d$ be a set of linearly independent vectors. Then $\text{span}(\vec{w}_1,\dots,\vec{w}_k)$ is a $k$-dimensional manifold in $\mathbb{R}^d.$ When $k=1$, the span is a line; for $k>1$ the span is a hyperplane.  The sphere unit sphere $\{\vec{x}\in\mathbb{R}^d:\,\|\vec{x}\|=1\}$ is a $d-1$ dimensional manifold.  For example, when $d=3$ manifold resembles the surface of the earth which locally looks like a portion of the 2-dimensional plane.  The Swiss roll from Example \@ref(exm:ex-swiss-roll) is a two-dimensional manifold in $\mathbb{R}^3$.
:::
...
Let's revisit the Swiss roll example (Example \@ref(exm:ex-swiss-roll)) where $A = (\pi/2,9\pi/2)\times (0,15)$.  We defined the map $\Psi:A\to \mathbb{R}^3$ as follows
...
## A Unifying View: Manifold Learning as Kernel PCA {#sec-unifying-view}
