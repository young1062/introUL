...
Unlike the supervised setting, however, UL algorithms are applied to datasets without (or ignoring) labels. In opposition the MNIST example above, you can think of have of a case where one has access to a large collection of $\vec{x}_i$, such as images, without any labels indicating the content(s) of the image.  Other examples include,
...
We'll focus on those cases where we have a collection of independent observations of (preprocessed) features stored in vectors $\vec{x}_1,\dots,\vec{x}_N\in\mathbb{R}^d$ where $d$ is typically large. This setting will be formalized in \@ref(ch-prob).  Broadly, UL learns patterns and similarities between the vectors which could allow us to 
