...
$$\mathbb{H}(f) = \int_{\mathcal{M}}\|\mathcal{H}^{tan}\|_F^2\, d\mu(\vec{x})$$ we have a measure of the average curvature of the function $f$. Here, $\mu(\vec{x})$ is the probability distribution of on the manifold fold (a measure for those with the requisite background). The utility of this quantity of $f$ is given in the following theorem.

::: {.theorem #hlle name="Null Space of Hessian Operator on Manifolds"}
Suppose $\mathcal{M} = \Psi(A)$ where $A$ is an open, connected subset of $\mathbb{R}^t$, and $\Psi$ is a locally isometric embedding of $A$ into $\mathbb{R}^d$. Then $\mathbb{H}(f)$ has a $t+1$ dimensional null space spanned by the constant function and a $t$-dimensional space of functions spanned by the original isometric coordinates in $A$.
:::
